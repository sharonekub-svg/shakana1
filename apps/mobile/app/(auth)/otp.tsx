...
export default function Otp() {
  ...
  const submit = async () => {
    ...
    try {
      await verify.mutateAsync({ phone: String(phoneE164), token: code });
      router.replace('/(auth)/name');
    } catch (e) {
      pushToast(e instanceof Error ? e.message : 'קוד שגוי', 'error');
  ...
  const handleResend = async () => {
    try {
      ...
      pushToast(e instanceof Error ? e.message : 'שגיאה בשליחה מחדש', 'error');
    }
  };
  ...
  return (
    <ScreenBase style={{ paddingTop: 20, paddingBottom: 36 }}>
      ...
      <View style={{ marginBottom: 32 }}>
        ...
        <Text style={styles.subtitle}>נשלח ל-{display ?? phoneE164}</Text>
      </View>
      ...
      <View style={styles.resendBlock}>
        <Text style={styles.resendPrompt}>לא קיבלת קוד?</Text>
        {resent ? (
          <Text style={styles.resentOk}>✓ קוד חדש נשלח</Text>
        ) : (
          <TextBtn label="שלח שוב" onPress={handleResend} />
        )}
      </View>

      <View style={{ flex: 1 }} />

      <PrimaryBtn label="אמת קוד" onPress={submit} disabled={!full} loading={verify.isPending} />
    </ScreenBase>
  );
}
