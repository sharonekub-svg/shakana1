...
export default function Pay() {
  ...
  const perPerson = order
    ? Math.ceil(order.product_price_agorot / order.max_participants)
    : 0;
  ...
  const go = async () => {
    try {
      ...
      pushToast(e instanceof Error ? e.message : 'התשלום נכשל', 'error');
    }
  };
      ...
      {!order ? (
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.card}>
            <Text style={styles.lead}>חלקך בהזמנה</Text>
            <Text style={styles.amount}>{formatAgorot(perPerson)}</Text>
            <Text style={styles.note}>
              הסכום יוחזק בנאמנות עד שההזמנה תגיע לבניין שלך
            </Text>
          </View>

          <View style={{ flex: 1 }} />

          <PrimaryBtn label="אישור תשלום" onPress={go} loading={pay.isPending} />
        </View>
      )}
    </ScreenBase>
  );
